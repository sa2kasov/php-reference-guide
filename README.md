...
`continue` и `break` принимают числовой параметр, сообщающей интерпретатору какое кол-во вложенных структур надо прервать.
...
Если в качестве второго параметра передать еще и 1 `count(myArray, 1)`, то функция подсчитает все вложенные элементы, если это многомерный массив.
