...
MySQL – свободная реляционная СУБД распространяемая под лицензией [GNU GPL](https://ru.wikipedia.org/wiki/GNU_General_Public_License) и под собственной коммерческой лицензией. Изначально разрабатывалась в шведской компании MySQL AB. 26 февраля 2008 года компания Sun Microsystems приобрела MySQL AB, а позднее 27 января 2010 года компания Oracle приобрела Sun Microsystems включив MySQL в свою линейку СУБД.
...
SET NAMES 'cp866'
:: Показать таблицу
SHOW TABLES :: Показывает существующие таблицы в БД
...
Конец запроса разделяется `;` (точка с запятой или `\g`). Если результат выборки не помещается в окно, то можно в конце добавить `\G`.
