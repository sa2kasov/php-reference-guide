...
Внутри абстрактного класса может быть и абстрактный метод. Абстрактный метод – это метод без реализаций (фигурных скобок). Он должен быть перегружен в классе наследника (иначе будет ошибка уровня _Fatal error_).
