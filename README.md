...
Класс определяется ключевым словом `class` за котороым следует его имя и пара фигурных скобок внутри которых могут содержаться свойства и методы этого класса.
...
Области видимости свойств, методов или констант могут быть определены как:

* `public` (_публичный_, _общедоступный_) – доступ к свойству или методу разрешён отовсюду;
* `protected` (_защищённый_) – разрешает доступ в самом классе, а также в наследуемых и родительских классах;
...
`$this` – указатель на запрашиваемый объект. Чтобы явно указать какой объект имеется ввиду. Доступ из метода к свойству объекта. Вызов метода из метода также происходит через ключевое слово `$this`.

```php
<?php
class Car{
  public function getSpeed(){
    $this->avgSpeed(); // Обращение к методу
  }
  public function avgSpeed(){
    'Скорость = ' . $this->speed; // Обращение к свойству
  }
}
?>
```
...
_Наследование (полиморфизм)_ – дочерний класс наследует все публичные и защищенные свойства и методы родительского класса. При объявлении класса, нужно указать какой класс он наследует (`extends`).
